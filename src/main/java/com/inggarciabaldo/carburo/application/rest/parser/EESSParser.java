...
import com.inggarciabaldo.carburo.model.entity.eess.PrecioCombustible;
...
import com.inggarciabaldo.carburo.persistence.JPAUtil;
import com.inggarciabaldo.carburo.persistence.repository.CombustibleRepository;
import com.inggarciabaldo.carburo.persistence.repository.EESSRepository;
import com.inggarciabaldo.carburo.persistence.repository.PrecioCombustibleRepository;
...
public class EESSParser {
    ...
    private static final Properties claves = PropertyLoader.getInstance().getJsonKeysProperties();
    ...
    public ES parse(JSONObject item, LocalDate fecha, Combustible combustible) {
        ...
        if (eessExistentes.contains(extCode))
            return null;
        else {
     ...
     * @param combustible Combustible específico a parsear (opcional). Null = todos.
     */
    private void parsePrecios(JSONObject item, ES ES, LocalDate fecha, Combustible combustible) {
        if (combustible == null) {
    ...
    private void parsePreciosTodosCombustibles(JSONObject item, ES ES, LocalDate fecha) {
        List<Combustible> combustibles = repoCombustible.findAll();
     ...
     */
    private void parsePrecioCombustible(JSONObject item, ES ES, LocalDate fecha, Combustible c) {
     ...
     */
    private BigDecimal parsearPrecio(JSONObject item, String jsonKey, Combustible combustible) {
        String precioStr = item.optString(jsonKey, "").replace(",", ".").trim();
        if (precioStr.isEmpty()) return null;
        try {
            return new BigDecimal(precioStr);
        } catch (NumberFormatException e) {
            parseLog.error("Precio inválido para {}: {}", combustible.getDenominacion(), precioStr, e);
            return null;
        }
    }
     ...
     */
    private void persistirPrecio(ES ES, Combustible combustible, LocalDate fecha, BigDecimal precio) {
        PrecioCombustible existente = repoPrecioCombustible.findByEESSAndCombustibleAndFecha(ES, combustible, fecha);
        ...
        if (existente != null) {
            ...
            Combustible managedCombustible = em.getReference(Combustible.class, combustible.getId());

            PrecioCombustible nuevo = new PrecioCombustible(managedES, managedCombustible, fecha, precio);
    ...
    private BigDecimal parseCodigoPostal(JSONObject item) {
        ...
        if (cpStr.isEmpty()) throw new IllegalArgumentException("Código postal vacío o nulo.");
        try {
            int cp = Integer.parseInt(cpStr);
            if (cp < 1000 || cp > 52999) throw new IllegalArgumentException("Código postal fuera de rango: " + cp);
    ...
    private Provincia parseProvincia(JSONObject item) {
        BigDecimal extCodeProvincia = item.optBigDecimal(IDPROVINCIA, BigDecimal.valueOf(-1));
        if (extCodeProvincia.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("ExtCode Provincia inválido: " + extCodeProvincia);
        }
        if (!provinciasMap.containsKey(extCodeProvincia))
            throw new IllegalArgumentException("Provincia no encontrada: " + extCodeProvincia);
        return provinciasMap.get(extCodeProvincia);
    }

    private Municipio parseMunicipio(JSONObject item) {
        BigDecimal extCodeMunicipio = item.optBigDecimal(IDMUNICIPIO, BigDecimal.valueOf(-1));
        if (extCodeMunicipio.compareTo(BigDecimal.ZERO) <= 0)
            throw new IllegalArgumentException("ExtCode Municipio inválido: " + extCodeMunicipio);
        // Buscamos el municipio en la base de datos
        if (!municipiosMap.containsKey(extCodeMunicipio))
            throw new IllegalArgumentException("Municipio no encontrado: " + extCodeMunicipio);
    ...
    private BigDecimal parseLongitud(JSONObject item) {
        ...
        if (lonStr.isEmpty()) throw new IllegalArgumentException("Longitud vacía o nula.");
