...
public class UpdateAllPrecioCombustibles implements Command<Integer> {

	private final Collection<PrecioCombustible> precioCombustibles;

	public UpdateAllPrecioCombustibles(Collection<PrecioCombustible> precioCombustibles) {
	...
	public Integer execute() {
		...
		Loggers.DB.info(
				"COMIENZO de ACTUALIZACIÓN MASIVA de Precio-Combustible. Pendientes: {}.",
				records.size());

		long inicio = System.currentTimeMillis();
		int insertadas = 0;
		for (PrecioCombustibleRecord record : records) {
			gateway.update(record); // TODO mejorar a todo de golpe
			insertadas++;
		}
	...
	private void logInicioProceso() {
		Loggers.DB.info(
				"COMIENZA el proceso de ACTUALIZACIÓN MASIVA de Precio-Combustible con disponibilidad y precios. Total Precios-Combustibles: {}.",
	...
	private void logColeccionVacia() {
		Loggers.DB.info(
				"FINALIZA el proceso de ACTUALIZACIÓN MASIVA de Precio-Combustible. COLECCIÓN VACÍA.");
	}
